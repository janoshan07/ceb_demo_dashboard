...
public class PreviewService {
    ...
    static {
        FIELD_ALIASES.put("accountno",
                Arrays.asList("accountno", "accountnumber", "account_no", "acc_no", "accno", "account", "acctno"));
        FIELD_ALIASES.put("customername",
                Arrays.asList("customername", "name", "customer_name", "cust_name", "clientname", "consumername"));
        FIELD_ALIASES.put("customeraddress",
                Arrays.asList("customeraddress", "address", "customer_address", "cust_address", "addr"));
        FIELD_ALIASES.put("mobileno", Arrays.asList("mobileno", "mobile", "phone", "contact", "mobile_no", "tel",
                "phoneno", "mobilenumber", "mobile_number", "mobnum"));
        FIELD_ALIASES.put("refno", Arrays.asList("refno", "ref_no", "referenceno", "referencenumber", "reference",
                "billno", "invoiceno", "refnum"));
        FIELD_ALIASES.put("fromdate", Arrays.asList("fromdate", "from_date", "startdate", "start_date", "billingfrom",
                "datefrom", "period_from", "from", "billperiod", "period"));
        FIELD_ALIASES.put("todate",
                Arrays.asList("todate", "to_date", "enddate", "end_date", "billingto", "dateto", "period_to", "to"));
        FIELD_ALIASES.put("imports", Arrays.asList("imports", "import", "importunits", "import_units", "consumption",
        ...
        FIELD_ALIASES.put("bankaccountno",
                Arrays.asList("bankaccountno", "bankaccount", "bank_account_no", "bank_acc", "accountnumber_bank"));
        FIELD_ALIASES.put("agreementdate",
                Arrays.asList("agreementdate", "agreement_date", "agreement", "contractdate", "installdate"));
        FIELD_ALIASES.put("panelcapacity",
                Arrays.asList("panelcapacity", "panel_capacity", "capacity", "kw", "solarcapacity", "panel_kw"));
        FIELD_ALIASES.put("solartype",
                Arrays.asList("solartype", "solar_type", "systemtype", "system_type", "nettype", "type"));

        // Add Step 2/3 specific fields
        FIELD_ALIASES.put("prevreadingdate", Arrays.asList("prevreadingdate", "prev_reading_date", "prvrdgdate",
                "previousreadingdate", "previous_reading_date", "prv_rdg_date"));
        FIELD_ALIASES.put("currreadingdate", Arrays.asList("currreadingdate", "curr_reading_date", "crntrdgdate",
                "currentreadingdate", "current_reading_date", "crnt_rdg_date"));
        FIELD_ALIASES.put("billsetoff", Arrays.asList("billsetoff", "bill_set_off", "setoff", "billoutstanding",
                "billoutstdsetoff", "billoutstd", "outstanding", "outstandingsetoff"));
    ...
    private static final Set<String> HEADER_KEYWORDS = new HashSet<>(Arrays.asList(
            ...
            "mobileno", "mobile", "phone", "contact", "refno", "referenceno", "fromdate", "startdate", "todate",
            "enddate",
            "imports", "import", "importunits", "exports", "export", "exportunits", "unitcost", "rate", "totalamount",
            "total", "amount", "billingmode", "expcode", "exportcode", "mode", "bankcode", "branchcode",
            "bankaccountno",
            "bankaccount", "agreementdate", "panelcapacity", "capacity", "solartype", "nettype", "costcode",
            "cost_code",
        ...
        try (InputStream is = new ByteArrayInputStream(fileData);
            ...
            for (int sheetIdx = 0; sheetIdx < numSheets; sheetIdx++) {
                ...
                boolean hasSubHeader = headerRowIdx + 1 <= sheet.getLastRowNum()
                        && isSubHeaderRow(sheet, headerRowIdx + 1);

                // Auto-detect logical column mapping using the detected header row and
                // sub-header row presence
                Map<String, Integer> colIndices = autoDetectColumns(sheet, headerRowIdx);

                // Parse raw headers from the detected header rows, merging parent + sub-headers
                // if present
                ...
                for (int col = 0; col < lastCellNum; col++) {
                    ...
                    if (!trimmedHdr.isEmpty()) {
                        ...
                        if (count > 1) {
                            // If it's a duplicate of "Account No", label the second one as "Account No:
                            // (Bank)"
                ...
                boolean hasCustomerCols = !hasBillingCols
                        && CUSTOMER_REQUIRED.stream().allMatch(colIndices::containsKey);
                ...
                for (int r = dataStartRow; r <= lastRowNum; r++) {
                    ...
                    if (hasBillingCols) {
                        ...
                        ExcelValidationService.RowValidationResult rowResult = excelValidationService.validateRow(
                                sheetName, r + 1,
                                detectedType,
                        ...
                        if (fromDate != null && accountNo != null && !accountNo.isEmpty()) {
                            processedRecordsInUpload
                                    .add(accountNo + "|" + fromDate.getYear() + "|" + fromDate.getMonthValue());
                        }
                        ...
                        if ("DUPLICATE".equals(status) && fromDate != null && accountNo != null
                                && !accountNo.isEmpty()) {
                            List<com.ceb.billing.entities.BillingRecord> dbRecords = billingRecordRepository
                                    .findByCustomerAccountNoAndFromDateYearAndMonth(accountNo, fromDate.getYear(),
                            ...
                            if (dbRecords != null && !dbRecords.isEmpty()) {
                                ...
                                dbRecordMap.put("customerName",
                                        firstDb.getCustomer() != null ? firstDb.getCustomer().getCustomerName() : "");
                                dbRecordMap.put("fromDate",
                                        firstDb.getFromDate() != null ? firstDb.getFromDate().toString() : "");
                                dbRecordMap.put("toDate",
                                        firstDb.getToDate() != null ? firstDb.getToDate().toString() : "");
                        ...
                        if (rowResult.hasWarnings()) {
                            sheetWarnings++;
                            warningCount++;
                        }
                        if (rowResult.hasDuplicate()) {
                            sheetDuplicates++;
                            duplicateCount++;
                        }
                        ...
                        ExcelValidationService.RowValidationResult rowResult = excelValidationService
                                .validateCustomerRow(
                                        sheetName, r + 1,
                                        accountNo,
                                        customerName,
    ...
    /**
     * Intelligently detect column indices from actual headers, regardless of order
     * or naming.
    ...
    public Map<String, Integer> autoDetectColumns(Sheet sheet, int headerRowIdx) {
        ...
        // Build clean list of actual headers (index -> clean header) to allow duplicate
        // headers
        ...
        // Handle duplicate accountno columns mapping (e.g. two columns with "Account
        // No:")
    ...
    private boolean isSubstringMatch(String fieldName, String cleanActual) {
        if (fieldName.length() >= 5 && cleanActual.contains(fieldName))
            return true;
        if (cleanActual.length() >= 5 && fieldName.contains(cleanActual))
            return true;
        return false;
    }
    ...
    private String getDateVal(Row row, Integer idx) {
        ...
        String[] patterns = { "yyyy-MM-dd", "M/d/yyyy", "d/M/yyyy", "MM/dd/yyyy", "dd/MM/yyyy",
                "dd-MM-yyyy", "MM-dd-yyyy", "yyyy/MM/dd", "yyyy.MM.dd", "dd.MM.yyyy", "MM.dd.yyyy", "yyyy.M.d",
                "d.M.yyyy" };
    ...
    private LocalDate parseDate(Row row, Integer idx) {
        ...
        String[] patterns = { "yyyy-MM-dd", "M/d/yyyy", "d/M/yyyy", "MM/dd/yyyy",
                "dd/MM/yyyy", "dd-MM-yyyy", "MM-dd-yyyy", "yyyy/MM/dd", "yyyy.MM.dd", "dd.MM.yyyy", "MM.dd.yyyy",
                "yyyy.M.d", "d.M.yyyy" };
        for (String pattern : patterns) {
            try {
